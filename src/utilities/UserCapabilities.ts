...
export class UserCapabilities {
     ...
     */
    /**
     * Checks whenever browser supports native HLS playback
     */
    public static hasHLSSupport(videoObject?: HTMLVideoElement | null): boolean {
        const video = videoObject || document.createElement('video');
        return Boolean(
    ...
    /**
     * Checks for WebCodecs API support
     */
    public static hasWebCodecsSupport(): boolean {
     ...
     */
    public static hasMMSSupport(): boolean {
        // @ts-ignore
        return !!(window.ManagedMediaSource);
    }

    /**
     * Returns supported video codecs for MSE
     */
    public static getSupportedVideoCodecs(): string[] {
        const codecs: string[] = [];

        const videoTests: Record<string, string> = {
            'h264': 'video/mp4; codecs="avc1.42E01E"',
            'h264-high': 'video/mp4; codecs="avc1.64001E"',
        ...
        if (typeof MediaSource !== 'undefined' && MediaSource.isTypeSupported) {
            for (const name in videoTests) {
                if (videoTests.hasOwnProperty(name)) {
                    if (MediaSource.isTypeSupported(videoTests[name])) {
                        codecs.push(name);
                    }
                }
            }
    ...
    /**
     * Returns supported audio codecs for MSE
     */
    public static getSupportedAudioCodecs(): string[] {
        const codecs: string[] = [];

        const audioTests: Record<string, string> = {
        ...
        if (typeof MediaSource !== 'undefined' && MediaSource.isTypeSupported) {
            for (const name in audioTests) {
                if (audioTests.hasOwnProperty(name)) {
                    if (MediaSource.isTypeSupported(audioTests[name])) {
                        codecs.push(name);
                    }
                }
            }
